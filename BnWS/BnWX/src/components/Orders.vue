...
export default {
  ...
  methods: {
    ...
    deleteOrder(orderId) {
      var url = this.apiServer + 'zy/deleteOrder';
      var data = {
        orderId: orderId
      };
      var vm = this;
    ...
    payOrder(orderId) {
      var url = this.apiServer + 'zy/payOrder';
      var data = {
        orderId: orderId
      };
      var vm = this;
    ...
    displayStatus(s) {
      var l = this.allStatus.filter(function(x) {
        return x.Id == s;
      });
      if (l.length > 0) {
  ...
  computed: {
    showOrders() {
      ...
      for (var i = 0; i < olist.length; i++) {
        ...
        vo.list.push({
          label: '座位个数',
          value: o.Positions == null ? 0 : o.Positions.length
        })
        vo.list.push({
          label: '订单状态',
          value: vm.displayStatus(o.Status)
        })
        vo.actions = [];
        if (!vm.historyOrder) {
          if (o.Status == 1) {
